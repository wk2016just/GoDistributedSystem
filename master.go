...
func Sequential(jobName string, files []string, nreduce int,
	...
	mr = newMaster("master") //创建1个master结构体，然后运行1个goroutine来执行mr.run函数
...
func (mr *Master) run(jobName string, files []string, nreduce int,
	schedule func(phase jobPhase),  //mr.run函数来执行具体的运行任务，其中重点是schedule(mapPhase)和schedule(reducePhase)
	finish func(),
) {
	mr.jobName = jobName
	mr.files = files
	...
	schedule(mapPhase)  //这里schedule函数是调用时的1个匿名函数，主要功能是区分任务类型，对于map任务执行doMap函数，对于reduce任务执行doReduce函数
	...
	mr.doneChannel <- true //最后向doneChannel信道中写入true标识这次任务的结束
}
